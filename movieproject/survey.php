...
function getRecommendedMovie($genres, $movie_age) {
    $apiKey = 'e0972b7e';  // Replace with your actual API key
    $movieTitles = [
  ...
  <div id="recommendationModal" class="modal">
    ...
    <script>
        ...
        document.getElementById('surveyForm').addEventListener('submit', function(e) {
            ...
            fetch('', { // Send data to current file using fetch API
                method: 'POST',
                body: formData
            })
    ...
    btn.textContent = 'Loading...'; // Show loading state

    const formData = new FormData(document.getElementById('surveyForm'));
    fetch('', { // Send request to current file
        method: 'POST',
        body: formData
    })
    ...
    .then(data => {
        ...
        if (data.success && data.movie) {
            showModal(data.movie); // Display new movie suggestion
        } else {
